...
__vue_options__.__file = "F:\\buyCourse\\src\\components\\HelloWorld.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
...
exports.default = {
  ...
  data: function data() {
    return {
      ...
      hideOrderPage: true, //付款码显示时隐藏订单部分元素
  ...
  methods: {
    ...
    getCourseList: function getCourseList() {
      var me = this;
      var GET_URL = me.webHost + '/api/trainerApp/saleCourse/courseList/' + me.centerId + '/' + me.traineeId;
      stream.fetch({
        method: 'GET',
        url: GET_URL,
        type: 'json',
        ...
        if (!ret.ok) {
          ...
          if (ret.data.status == 0) {
            if (ret.data.data == null || ret.data.data == undefined || ret.data.data == '') {
              ...
              if (trainee.photo == undefined || trainee.photo == null || trainee.photo == '') {
              ...
              if (course.notes == undefined || course.notes == '' || course.notes) {
                me.isNotes = false;
              } else {
                me.isNotes = true;
        ...
        if (course.assistSellerId == undefined || course.assistSellerId == '' || course.assistSellerId == null) {
          ...
          if (course.saleType == 3) {
            ...
            for (var i = 0; i < me.helpSaleList.length; i++) {
              if (course.assistSellerId == me.helpSaleList[i].id) {
                me.helpSaleId = me.helpSaleList[i].id;
        ...
        for (var i = 0; i < that.list.length; i++) {
          if (that.list[i].checked) {
            ...
            if (that.list[i].courseTypeId === 2) {
              that.effectiveDay = that.list[i].effectiveDays;
            }
          }
        }
        if (that.courseId == '') {
    ...
    toOrder: function toOrder() {
      ...
      if (that.types == 2) {
        if (that.minPrice == undefined) {
          ...
          if (that.money < that.minPrice) {
            TrainerNativeModule.toast('课程价格不低于' + that.minPrice + '元！');
            return;
          }
        }
        if (that.saleTypeId == '' || that.effectiveType == '' || that.money == '') {
      ...
      if (that.types == 1) {
        ...
        if (that.minPrice == undefined) {
          ...
          if (needPrice < that.minPrice || needPrice < 0) {
            TrainerNativeModule.toast("请输入合法的实收金额!");
            return;
          }
        }
        if (that.saleTypeId == '' || that.money == '' || that.num == '') {
          TrainerNativeModule.toast("请填写必填项");
          return;
        }
      }
    ...
    generateOrder: function generateOrder() {
      var that = this;
      var d = new Date(that.year + '-' + that.month + '-' + that.day);
      if (that.payMoney < 0) {
        TrainerNativeModule.toast("实收金额不能小于0!");
        return;
      }
      var assistCommissionAmount;
      var amount;
      var saleRatio;
      var depositIds;
      var courseBuyUnitPrice;
      var activatedMode;
      var startDate;
      var endDate;
      var count;
      var assistSellerId;
      var payMethod1Amount;
      var createdTs = d.getTime(d);
      var assistSellerName;
      //处理协助金额
      if (that.precentItem == '请选择协助比例' || that.helpSaleItem == '选择协助销售') {
        assistCommissionAmount = '';
      } else {
        for (var i = 0; i < that.precentList.length; i++) {
          if (that.precentList[i].checked) {
            var pre = Number(that.precentList[i].id / 10);
            // modal.toast({ message:that.depositAsTrainerPerformance, duration: 1 });
            if (!that.depositAsTrainerPerformance) {
              assistCommissionAmount = Number(that.money) * pre;
              assistCommissionAmount = assistCommissionAmount.toFixed;
            } else {
              if (that.pannyItem != '选择可用定金') {
      ...
      if (that.helpSaleItem == '选择协助销售') {
        assistSellerId = that.helpSaleId;
      } else {
        assistSellerName = that.helpSaleItem;
      }
      //出理协助比例
      if (that.precentItem == '请选择协助比例') {
        saleRatio = '';
      } else {
        for (var i = 0; i < that.precentList.length; i++) {
          if (that.precentList[i].checked) {
      ...
      if (that.helpSaleItem == '选择协助销售' && that.precentItem != '请选择协助比例') {
      ...
      // modal.toast({ message:amount+'-'+payMethod1Amount, duration: 1 });
      //处理每节课价格
      if (that.types == 1) {
      ...
      // modal.toast({ message:depositIds, duration: 10 });
      ...
      //   createdTs:that.defaultTitle,
      //   activatedMode:activatedMode,
      //   createdTs:createdTs,
      //   courseName:that.courseItem,
      //   assistSellerName:assistSellerName
      // })
      modal.toast({ message: assistCommissionAmount, duration: 10 });
      //    var url="";
      //    var method='';
      //    if(that.isEdit){
      //      url=that.webHost+'/api/trainerApp/saleCourse/editPerCourse/'+that.orderId;
      //      that.isEdit=false;
      //       method='PUT';
      //    }else{
      //       url=that.webHost+'/api/trainerApp/saleCourse/perCourse/'+that.centerId;
      //       method='POST';
      //    }
      //   stream.fetch({
      //     method:method,
      ...
      //     body:data
      //  }, function(ret) {
      //   // modal.toast({ message:ret, duration: 10 });
      //     if(ret.data.status==0){
      //         that.getOrder(); 
      //        setTimeout(function(){
      //           that.showCoursePage=false;
    ...
    getMoney: function getMoney(event) {
      ...
      //   this.payMoney=this.money;
      // }else{
      //   var payMoney=Number(this.money)-Number(this.pannyItem);
    ...
    aliPay: function aliPay() {
      ...
      stream.fetch({
        method: "GET",
        type: 'json',
        url: that.webHost + '/api/trainerApp/saleCourse/' + that.centerId + '/getAggregateOrder/' + that.orderId + '/' + 4,
        headers: {
          "Content-Type": 'application/json',
          'X-AUTH-TOKEN': that.token
        }
    ...
    wechatpay: function wechatpay() {
      ...
      stream.fetch({
        method: "GET",
        type: 'json',
        url: that.webHost + '/api/trainerApp/saleCourse/' + that.centerId + '/getAggregateOrder/' + that.orderId + '/' + 5,
        headers: {
          "Content-Type": 'application/json',
          'X-AUTH-TOKEN': that.token
        }
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
  ...
  }, [_vm._v(_vm._s(_vm.realPrice))]), (_vm.effectiveDays != undefined && _vm.effectiveDays != null && _vm.effectiveDays != '') ? _c('text', {
    staticClass: ["realDay"]
    ...
    staticClass: ["txt"]
  }, [_vm._v("请选择私教课")]), _c('image', {
    staticClass: ["chooseImg"],
    attrs: {
      "src": "http://bocai-center.oss-cn-hangzhou.aliyuncs.com/center_manager/static_img/Rectanglex.png"
    }
  })])]), ((_vm.showButtom)) ? _c('div', {
  ...
  })]) : _vm._e()])])]), ((_vm.isShowchooseCourse)) ? _c('div', {
    staticClass: ["bg"],
    style: {
      height: _vm.height
    }
  ...
  }, [_vm._v(_vm._s(_vm.orderPeoplePhone || '--'))])])])]), ((_vm.hideOrderPage)) ? _c('div', {
    staticClass: ["orderMess"]
  }, [_c('div', {
    staticClass: ["orderTitle"]
  }, [_c('text', {
    staticClass: ["leftTxt"]
  }, [_vm._v("订单信息")]), (_vm.isFinish) ? _c('div', {
    staticClass: ["editBtn"],
    on: {
      "click": _vm.editOrder
    }
    ...
    staticClass: ["editImg"],
    attrs: {
      "src": "https://bocai-center.oss-cn-hangzhou.aliyuncs.com/center_manager/static_img/edit.png"
    }
  })]) : _c('div', {
    staticClass: ["payFinishs"]
  }, [_c('text', {
    staticClass: ["payFinishResult"]
  }, [_vm._v("已付款")])])]), _c('div', {
  ...
  }, [_vm._v(_vm._s(_vm.orderRealPrice) + " 元")])])])]) : _vm._e()]), _c('div', {
    staticClass: ["checkIn"],
    style: {
  ...
  }, [_vm._v("发起付款")])]), ((_vm.showPayCode)) ? _c('div', {
    staticClass: ["bg"],
    style: {
      height: _vm.height
    }
  }, [_c('div', {
    staticClass: ["payCodeBox"]
  }, [_c('div', {
    staticClass: ["codeAndprice"]
  }, [_c('text', {
    staticClass: ["price"]
  }, [_vm._v("需支付￥" + _vm._s(_vm.needPayMoney) + "元")]), _c('image', {
    staticClass: ["qrCode"],
    attrs: {
      "src": _vm.qrCode
    }
    ...
    staticClass: ["tips"]
  }, [_vm._v("扫一扫完成支付")])]), _c('div', {
    staticClass: ["payResultBtn"]
  }, [_c('text', {
    staticClass: ["hideCode"],
    on: {
      "click": _vm.cancelPay
    }
  }, [_vm._v("取消")]), _c('text', {
  ...
  }, [_vm._v("已支付")])])])]) : _vm._e(), ((_vm.showPayStyle)) ? _c('div', {
    staticClass: ["bg"],
    style: {
      height: _vm.height
    }
    ...
    staticClass: ["payTxt"]
  }, [_vm._v("微信")])])])])]) : _vm._e()]), _c('wxc-loading', {
    attrs: {
      "show": _vm.isShow
    }
  })], 1)
...
__vue_options__.__file = "F:\\buyCourse\\src\\payOrder\\payOrder.vue"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
__vue_options__.__file = "F:\\buyCourse\\src\\index.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
