...
exports.default = {
  ...
  data: function data() {
    return {
      ...
      month: '',
      day: '',
      saleList: [{ name: '新购课', id: 1, checked: false }, { name: '续课', id: 2, checked: false }, { name: 'pos销售', id: 3, checked: false }, { name: '场地开发', id: 4, checked: false }, { name: '交叉课', id: 5, checked: false }, { name: '会员转介绍', id: 8, checked: false }, { name: '体验课出单', id: 9, checked: false }, { name: '其他', id: 16, checked: false }],
      showCoursePage: true,
      notes: '',
      money: '',
      showPayStyle: false,
      ...
      precentId: '',
      precentItem: '请选择协助比例',
      precentList: [{ name: '分成10%', id: 1, checked: false }, { name: '分成20%', id: 2, checked: false }, { name: '分成30%', id: 3, checked: false }, { name: '分成40%', id: 4, checked: false }, { name: '分成50%', id: 5, checked: false }, { name: '分成60%', id: 6, checked: false }, { name: '分成70%', id: 7, checked: false }, { name: '分成80%', id: 8, checked: false }, { name: '分成90%', id: 9, checked: false }],
      consultantsList: [],
      minPrice: null,
      payMoney: '0',
      traineeId: '33',
      depositIdsList: [],
      depositAsTrainerPerformance: null,
      showChooseCourse: "hidden",
      realPrice: '',
      effectiveDays: '',
      coachList: [],
      isShow: false
  ...
  methods: {
    getCourseList: function getCourseList() {
      var me = this;
      var GET_URL = me.webHost + '/api/trainerApp/saleCourse/courseList/' + me.centerId + '/' + me.trainerId;
      stream.fetch({
        method: 'GET',
        url: GET_URL,
        type: 'json',
        headers: {
          'X-AUTH-TOKEN': 'eyJuYW1lIjoiUmVkQTFlcnQiLCJwaG9uZSI6IjE1ODg4ODMwODQ4IiwiYWNjb3VudElkIjo1MzE0MzR9.YVUVqYttJUd85cj0GMBX9jZQCUCvvvDrB39AsDLvd3Y='
        }
      }, function (ret) {
        // modal.toast({ message:ret.data.data.trainers, duration: 10});
        ...
        me.list = list;
        me.depositAsTrainerPerformance = ret.data.data.depositAsTrainerPerformance;
      }, function (response) {});
    },
    ...
    hideCourseList: function hideCourseList() {
      ...
      if (that.data == '选择课程') {
        if (that.courseId == '') {
          modal.toast({ message: '请选择课程', duration: 1 });
        } else {
        ...
        if (that.saleTypeId == '') {
          modal.toast({ message: '请选择销售类型', duration: 1 });
        } else {
          that.componentVisibility = 'visible';
          that.showButtom = true;
          that.isShowchooseCourse = !that.isShowchooseCourse;
    ...
    getCourseItem: function getCourseItem() {
      ...
      if (that.data == '选择课程') {
        for (var i = 0; i < that.list.length; i++) {
          if (that.list[i].checked) {
            ...
            // modal.toast({ message:that.list[i], duration: 10 });
      ...
      if (that.data == '销售类型') {
        ...
        // modal.toast({ message:that.helpSaleList, duration: 10 });
        if (that.saleTypeId == '') {
          modal.toast({ message: '请选择销售类型', duration: 1 });
        } else {
          that.componentVisibility = 'visible';
      ...
      if (that.data == '协助销售') {
        for (var i = 0; i < that.list.length; i++) {
          if (that.list[i].checked) {
            that.helpSaleId = that.list[i].id;
    ...
    onblur: function onblur() {
      this.setTop = 0;
      modal.toast({ message: this.setTop, duration: 1 });
    },
    ...
    getpannyItem: function getpannyItem() {
      ...
      if (that.pannyItem == '选择可用定金') {
        that.payMoney = this.money;
        that.depositIdsList = [];
      } else {
        that.payMoney = that.money - that.pannyItem;
        if (that.payMoney < 0) {
          modal.toast({ message: "需支付不能小于0", duration: 1 });
          that.payMoney = 0;
        }
      }
      that.componentVisibility = 'visible';
    ...
    toOrder: function toOrder() {
      ...
      var that = this;
      that.isShow = true;
      // modal.toast({ message:typeof(that.minPrice), duration: 10});
      if (that.types == 2) {
        if (that.minPrice == undefined) {
          if (Number(that.money) < 0) {
            modal.toast({ message: "金额不能小于0！", duration: 1 });
            return;
          }
        } else {
          if (that.money < that.minPrice) {
            modal.toast({ message: "课程价格不低于" + that.minPrice + "元！", duration: 1 });
            return;
          }
        }
        if (that.saleTypeId == '' || that.effectiveType == '' || that.money == '') {
          modal.toast({ message: "请填写必填项", duration: 1 });
          return;
        }
      }
      if (that.types == 1) {
        var needPrice = (Number(that.money) / Number(that.num)).toFixed(0);
        if (that.num < 0 || that.num > 999) {
          modal.toast({ message: "请输入1-999之间的正整数节数！", duration: 1 });
          return;
        }
        if (that.minPrice == undefined) {
          if (that.money < 0) {
          ...
          if (needPrice < that.minPrice || needPrice < 0) {
            modal.toast({ message: "请输入合法的课程价格！", duration: 1 });
            return;
          }
        }
    ...
    generateOrder: function generateOrder() {
      ...
      if (that.payMoney < 0) {
        modal.toast({ message: "实收金额不能小于0", duration: 1 });
      }
      ...
      if (that.precentItem == '请选择协助比例' || that.helpSaleItem == '选择协助销售') {
        ...
        for (var i = 0; i < that.precentList.length; i++) {
          if (that.precentList[i].checked) {
            // modal.toast({ message:that.precentList[i].id/10, duration: 1 });
      ...
      if (that.pannyItem == '选择可用定金') {
        ...
        amount = Number(that.money) + Number(that.pannyItem);
        depositIds = that.set(that.depositIdsList);
        payMethod1Amount = Number(that.money) - Number(that.pannyItem);
      }
      //处理每节课价格
      if (that.types == 1) {
        courseBuyUnitPrice = (Number(that.money) / Number(that.num)).toFixed(2);
      ...
      if (that.types == 2) {
        ...
        if (that.effectiveType == 2) {
          ...
          if (that.endDate == '0000-00-00') {
            modal.toast({ message: '请选择生效日期', duration: 1 });
          } else {
      ...
      var data = JSON.stringify(_defineProperty({
      ...
      //     body:data
      // }, function(ret) {
      //      modal.toast({ message:ret.data, duration: 1 });
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    attrs: {
      "src": _vm.courseImg + '?x-oss-process=image/resize,h_200'
    }
  ...
  }, [_vm._v(_vm._s(_vm.realPrice))]), (_vm.effectiveDays != undefined) ? _c('text', {
    staticClass: ["realDay"]
    ...
    staticClass: ["title"]
  }, [_vm._v("确认订单")]), _vm._m(8)]), _c('div', {
    staticClass: ["orderContent"]
  }, [_vm._m(9), ((_vm.hideOrderPage)) ? _c('div', {
    staticClass: ["orderMess"]
  }, [_vm._m(10), _vm._m(11), ((_vm.pannyList.length > 0)) ? _c('div', {
    staticClass: ["earnestList"]
  }, [_vm._m(12), _vm._m(13), _vm._m(14)]) : _vm._e()]) : _vm._e()]), _c('div', {
    staticClass: ["checkIn"],
    style: {
  ...
  }, [_vm._v("发起付款")])]), ((_vm.showPayStyle)) ? _c('div', {
    staticClass: ["bg"],
    style: {
      height: _vm.height
    }
    ...
    staticClass: ["payTxt"]
  }, [_vm._v("微信")])])])])]) : _vm._e(), ((_vm.showPayCode)) ? _c('div', {
    staticClass: ["bg"],
    style: {
      height: _vm.height
    }
    ...
    staticClass: ["changePayStyle"]
  }, [_c('text', {
    staticClass: ["changeTxt"],
    on: {
      "click": _vm.changePay
    }
  }, [_vm._v("切换支付方式")])])]) : _vm._e()]), _c('wxc-loading', {
    attrs: {
      "show": _vm.isShow,
      "type": "trip"
    }
  ...
  return _c('div', {
    ...
    attrs: {
      "src": "https://bocai-center.oss-cn-hangzhou.aliyuncs.com/center_manager/static_img/defaultWoman.png"
    }
  }), _c('div', {
    ...
    attrs: {
      "src": "http://bocai-center.oss-cn-hangzhou.aliyuncs.com/center_manager/static_img/phone.png"
    }
  }), _c('text', {
  ...
  return _c('div', {
    ...
    staticClass: ["projectContent"]
  }, [_vm._v("指定日期生效")])]), _c('div', {
    staticClass: ["listChildren"]
  }, [_c('text', {
    staticClass: ["projectName"]
  }, [_vm._v("有效期")]), _c('text', {
    staticClass: ["projectContent"]
  }, [_vm._v("2018/10/10至2018/11/10")])]), _c('div', {
    staticClass: ["listChildren"]
  }, [_c('text', {
    staticClass: ["projectName"]
  }, [_vm._v("上课教练")]), _c('text', {
  ...
  return _c('div', {
    ...
    staticClass: ["price"]
  }, [_vm._v("需支付￥100元")]), _c('image', {
    staticClass: ["qrCode"],
    attrs: {
      "src": "https://bocai-center.oss-cn-hangzhou.aliyuncs.com/center_manager/static_img/qrcode.png"
    }
  }), _c('text', {
    staticClass: ["tips"]
  }, [_vm._v("扫一扫完成支付")])])
